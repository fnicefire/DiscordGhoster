...
def main():
    ...
    token = input(f"{Fore.CYAN}[?] Enter User Token: {Fore.WHITE}").strip()
    ...
    def attempt_user_login():
        ...
        try:
            print(f"{Fore.MAGENTA}[*] Attempting User Login...")
            client_instance.run(token)
            return True
        except discord.LoginFailure:
            print(f"\n{Fore.RED}[ERROR] Bot Token invalid or expired.")
            return False
        except Exception as e:
            print(f"\n{Fore.RED}[CRITICAL ERROR] Could not login as User.")
            return False

    if not attempt_user_login():
